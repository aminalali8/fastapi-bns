...
- 🎯 Entry points to execute your program using `python -m <fastapi_bns>` or `$ fastapi_bns` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/aminalali8/fastapi-bns/branch/main/graph/badge.svg?token=fastapi-bns_token_here)](https://codecov.io/gh/aminalali8/fastapi-bns)
[![CI](https://github.com/aminalali8/fastapi-bns/actions/workflows/main.yml/badge.svg)](https://github.com/aminalali8/fastapi-bns/actions/workflows/main.yml)

Awesome fastapi_bns created by aminalali8

## Install

from source
```bash
git clone https://github.com/aminalali8/fastapi-bns fastapi_bns
cd fastapi_bns
make install
```
...
```bash
pip install fastapi_bns
```

## Executing

```bash
$ fastapi_bns run --port 8080
```

or

```bash
python -m fastapi_bns run --port 8080
```

or

```bash
$ uvicorn fastapi_bns:app
```
...
```bash
❯ fastapi_bns --help
Usage: fastapi_bns [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_bns create-user --help
Usage: fastapi_bns create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_bns create-user admin admin --superuser
```
...
Run with `fastapi_bns run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_bns create-user`.
...
-----------------------------------------------------
fastapi_bns/__init__.py              4      0   100%
fastapi_bns/app.py                  16      1    94%
fastapi_bns/cli.py                  21      0   100%
fastapi_bns/config.py                5      0   100%
fastapi_bns/db.py                   10      0   100%
fastapi_bns/models/__init__.py       0      0   100%
fastapi_bns/models/content.py       47      1    98%
fastapi_bns/routes/__init__.py      11      0   100%
fastapi_bns/routes/content.py       52     25    52%
fastapi_bns/routes/security.py      15      1    93%
fastapi_bns/routes/user.py          52     26    50%
fastapi_bns/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fastapi_bns.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_bns/default.toml.

### As environment variables
```bash
export fastapi_bns_KEY=value
export fastapi_bns_KEY="@int 42"
export fastapi_bns_KEY="@jinja {{ this.db.uri }}"
export fastapi_bns_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastapi_bns_ENV=production fastapi_bns run
```
